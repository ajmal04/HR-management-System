...
export default class Announcement extends Component {
  ...
  fetchData = () => {
    ...
    Promise.all([
      ...
      axios.get("/api/departmentAnnouncements/college", {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      }),
    ])
      .then(([deptRes, annRes]) => {
  ...
  handleSubmit = (event) => {
    ...
    const data = {
      ...
      departmentId:
        selectedDepartment === ALL_DEPARTMENTS ? null : selectedDepartment,
    };

    axios
      .post("/api/departmentAnnouncements", data, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      })
      .then(() => {
        if (this._isMounted) {
  ...
  handleDeleteConfirm = () => {
    ...
    axios
      .delete(`/api/departmentAnnouncements/${announcementToDelete.id}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      })
      .then(() => {
        if (this._isMounted) {
