...
export default class ExpenseReport extends Component {
  constructor(props) {
    ...
    this.state = {
      expenses: [],
      allExpenses: [], // Store all expenses for client-side filtering
  ...
  fetchAllExpenses = () => {
    ...
    .then(res => {
      this.setState({
        ...
        this.filterExpensesByMonth(); // Filter initially with current date
      });
    })
    .catch(err => {
      this.setState({
  ...
  filterExpensesByMonth = () => {
    if (!this.state.selectedDate || this.state.allExpenses.length === 0) {
      this.setState({ expenses: [], totalAmount: 0 });
      return;
    }
    
    const filteredExpenses = this.state.allExpenses.filter(expense => {
        ...
        <Card>
          <Card.Header style={{ 
            ...
            <div className="panel-title">
              <strong>Department Expense Report</strong>
            </div>
                ...
                <MaterialTable
                  columns={[
                    ...
                    { title: 'Department', field: 'department.departmentName' },
                    {
                      ...
                      customFooter: () => (
                        <div>
                          <strong>Total: €{parseFloat(this.state.totalAmount).toFixed(2)}</strong>
                        </div>
                      )
                    }
                  ]}
